...
def mediacion_simple(days, symbol= None):
    # Cargar el CSV
    data = read_data(days, ['average', 'volume usd', 'open', 'symbol'])

    # Asegúrate de eliminar valores nulos
    data = data.dropna(subset=['average', 'volume usd', 'open'])
    ...
    M = data['open']  # 'open' como variable mediadora
    ...
    X_Y = sm.add_constant(data[['volume usd', 'open']])
    model_Y = sm.OLS(Y, X_Y).fit()

    # Resultados de ambos modelos
    print("\nResultados del modelo de mediación para la variable mediadora (open):")
    ...
    sns.scatterplot(x='volume usd', y='open', data=data, alpha=0.6)
    plt.title('Volumen de Trading vs Open')
    plt.xlabel('Volumen de Trading (USD)')
    plt.ylabel('Open')
    plt.grid()

    # Agregar línea de regresión para mediadora
    sns.regplot(x='volume usd', y='open', data=data, scatter=False, color='red')
    plt.show()

    # Gráfico de dispersión de la variable mediadora contra la variable dependiente
    plt.figure(figsize=(12, 6))
    sns.scatterplot(x='open', y='average', data=data, alpha=0.6)
    plt.title('Open vs Precio Promedio')
    plt.xlabel('Open')
    plt.ylabel('Precio Promedio')
    plt.grid()

    # Agregar línea de regresión para variable dependiente
    sns.regplot(x='open', y='average', data=data, scatter=False, color='red')
    plt.show()
